...
(() => {
  ...
  function route(m) {
    ...
    switch (m.t) {
      ...
      case 'session.spill': {
        if (window.__spillOff) return ok(rid, { chunks: [] });
        const text = window.__spillText !== undefined ? window.__spillText : '落盘缓存里的一段\\\\r\\\\n';
        const u = new TextEncoder().encode(text);
        let bin = ''; for (let i = 0; i < u.length; i++) bin += String.fromCharCode(u[i]);
        const resp = { bytes: u.length, data: btoa(bin), seq: window.__spillSeq === undefined ? 100 : window.__spillSeq, truncated: false };
        if (window.__spillDelayMs) { setTimeout(() => ok(rid, resp), window.__spillDelayMs); return; }
        return ok(rid, resp);
      }
      ...
      // 远端模型：读（__modelName / __modelRaw 控制）+ 写（__modelWriteFail 控制失败）+ 还原
      case 'hermes.model': return ok(rid, { raw: (window.__modelRaw && window.__modelRaw()) || '' });
      case 'hermes.model.set': {
        if (window.__modelWriteFail) return ok(rid, { raw: '@@ERR 写不进去（config.yaml 只读）' });
        const old0 = window.__modelName || 'deepseek-v4-flash';
        window.__modelName = m.model;
        return ok(rid, { raw: '@@OLD ' + old0 + '\\n@@NEW ' + m.model + '\\n@@OK 1\\n@@YAML ok\\n@@BAK /home/x/.hermes/config.yaml.hpk-bak-20260921-120000' });
      }
      case 'hermes.model.undo': {
        window.__modelName = 'deepseek-v4-flash';
        return ok(rid, { raw: '@@FROM config.yaml.hpk-bak-20260921-120000\\n@@MODEL deepseek-v4-flash\\n@@OK 1' });
      }
      // skill / 记忆：读原文（真算 sha256 + 真 base64）· 回写（带 sha 对账）· 存到手机（真算 sha）
      case 'skill.read': {
        if (window.__skillErr) return ok(rid, { raw: '@@ERR 读不到: 文件不存在' });
        const txt = window.__skillText !== undefined ? window.__skillText : '# 默认技能\\\\n一行正文\\\\n';
        ...
        crypto.subtle.digest('SHA-256', bytes).then((h) => {
          ...
          window.__skillSha = sha;
          ok(rid, { raw: '@@SIZE ' + bytes.length + '\\n@@SHA ' + sha + '\\n@@B64\\n' + btoa(bin) });
        });
        return;
      }
      case 'skill.write': {
        ...
        const bytes = dec(m.b64);
        const want = hex(bytes);
        if (window.__skillSha && m.sha && m.sha !== window.__skillSha) {
          return ok(rid, { raw: '@@ERR 远端文件已变（现在是 ' + window.__skillSha.slice(0, 12) + '，你手上的是 ' + String(m.sha).slice(0, 12) + '），先重新打开再改' });
        }
        return crypto.subtle.digest('SHA-256', bytes).then((h) => {
          const sha = hex(new Uint8Array(h));
          window.__skillSha = sha;
          window.__skillText = new TextDecoder('utf-8').decode(bytes);
          window.__wroteSkill = { path: m.path, sha: sha, size: bytes.length, text: window.__skillText };
          ok(rid, { raw: '@@OLD ' + String(m.sha || '').slice(0, 12) + '\\n@@NEW ' + sha + '\\n@@SIZE ' + bytes.length +
            '\\n@@BAK SKILL.md.hpk-bak-20260921-120000\\n@@OK 1' });
        });
      }
      case 'local.save': {
        const bin = atob(String(m.b64 || ''));
        const u = new Uint8Array(bin.length);
        for (let i = 0; i < bin.length; i++) u[i] = bin.charCodeAt(i);
        window.__savedFile = { name: m.name, size: u.length, b64: m.b64 };
        return crypto.subtle.digest('SHA-256', u).then((h) => {
          const sha = [...new Uint8Array(h)].map((x) => x.toString(16).padStart(2, '0')).join('');
          window.__savedFile.sha = sha;
          if (window.__saveFail) return ok(rid, { raw: '@@ERR 写不进手机：Permission denied' });
          ok(rid, { raw: '@@PATH Download/hermes-skills/' + m.name + '\\n@@SIZE ' + u.length + '\\n@@SHA ' + sha + '\\n@@OK ' + (window.__saveShaMismatch ? '0' : '1') });
        });
      }
      // 网络诊断：读窗口里的样本（驱动脚本把真样本塞进 window.__netSamples），可单独覆盖；顺手记下参数
      case 'net.ping': {
        window.__lastPing = { host: m.host, count: m.count };
        return ok(rid, {
          raw: window.__pingRaw !== undefined ? window.__pingRaw : String((window.__netSamples || {}).ping_loopback || ''),
          ms: window.__pingTookMs === undefined ? 620 : window.__pingTookMs
        });
      }
      case 'net.tcp': {
        window.__lastTcp = { host: m.host, port: m.port };
        return ok(rid, {
          raw: window.__tcpRaw !== undefined ? window.__tcpRaw : String((window.__netSamples || {}).tcp_本机22 || ''),
          ms: window.__tcpTookMs === undefined ? 9 : window.__tcpTookMs
        });
      }
