 ...
 *
 * 用法: node tools/testssh2.mjs --port 2223 --home /tmp/hpk-fakehome --password hpk-test-pass
 */
...
const PASSWORD = get('--password', 'hpk-test-pass');
...
const authKeysFile = path.join(HOME, '.ssh', 'authorizedKeys');
...
server.listen(PORT, '127.0.0.1', () => {
  ...
  console.log(`[testssh2] authorized_keys = ${authKeysFile}（current ${readAuthorized().split('\n').filter((l) => l.trim()).length} row）`);
});
