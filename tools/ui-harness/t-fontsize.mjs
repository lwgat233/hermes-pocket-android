...
export default {
  ...
  set: async (page) => {
    ...
    await page.evaluate(() => {
      ...
      if (!inp) throw new Error('设置页里没有 fontSize 输入框');
      inp.value = '24';
      inp.dispatchEvent(new Event('change', { bubbles: true }));
    });
    await page.waitForTimeout(700);
    const after = await read(page);
    return { before, after, 立刻生效: after.optFont === 24 && after.cssFont === '24px' && after.cols !== before.cols };
  },

  /* 重载页面 = 重开 App/WebView 重载：值必须还在（这是"默认保持就是当前界面的状态"） */
  afterReload: async (page) => {
    const r = await read(page);
    return { ...r, 重载后仍是24: r.optFont === 24 && r.cssFont === '24px' };
  }
};
